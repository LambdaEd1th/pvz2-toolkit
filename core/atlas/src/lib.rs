...
pub fn split_atlas(
    ...
    // 1. Read JSON
    let json_content = fs::read_to_string(json_path).context("Failed to read atlas JSON")?;
    let atlas: OfficialAtlas =
        serde_json::from_str(&json_content).context("Failed to parse atlas JSON")?;
    ...
    for res in atlas.resources {
        if let (Some(ax), Some(ay), Some(aw), Some(ah)) = (res.ax, res.ay, res.aw, res.ah) {
            ...
            sub_img
                .save(&save_path)
                .with_context(|| format!("Failed to save sprite {}", filename))?;
            count += 1;
        }
    }
