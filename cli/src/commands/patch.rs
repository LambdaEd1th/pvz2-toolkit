use anyhow::Result;
use clap::Subcommand;
use patch::{PatchError, decode, encode, io::RsbPatchReader};
use std::fs;
...
pub fn patch_extract(input: &Path, output: &Path) -> Result<()> {
    ...
    let header = reader.read_header().map_err(|e| {
        PatchError::RsbPatch(patch::error::RsbPatchError::InvalidFile(e.to_string()))
    })?;
    ...
    if let Some(header_diff) = reader.extract_header_diff(&header).map_err(|e| {
        PatchError::RsbPatch(patch::error::RsbPatchError::InvalidFile(format!(
